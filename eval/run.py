...
def build_from_labels(labels_csv: Path) -> list[Sample]:
    """Real samples: a labels.csv next to the images (see eval/README.md). Nothing is generated."""
    samples: list[Sample] = []
    with labels_csv.open(encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            path = (labels_csv.parent / row["file"]).resolve()
            form = {"name": row["form_name"]}
            if row.get("form_dob"):
                form["dob"] = row["form_dob"]
            samples.append(
                Sample(
                    sample_id=row["sample_id"],
                    event=POLICY_BY_NAME.get(row.get("event_policy", "adult_open"), ADULT),
                    form=form,
...
def run() -> int:
    ...
    parser.add_argument("--labels", type=Path, help="labels.csv of real samples; synthetic cards are used without it")
    parser.add_argument("--certs", type=Path, help="UIDAI certificate directory (defaults to the test one)")
    ...
    if args.labels:
        samples = build_from_labels(args.labels)
        certs = args.certs or Path("certs")
        print(f"{len(samples)} labelled samples from {args.labels}, certificates from {certs.resolve()}")
    else:
    ...
    source = (
        f"real labelled samples ({args.labels})"
        if args.labels
        else "synthetic SPECIMEN cards with a TEST signing key, not a substitute for real IDs"
    )
    report = summarise(rows, audit, source)
    (ROOT / "results.md").write_text(report, encoding="utf-8")
    print("\n" + report)
    return 0


def summarise(rows: list[tuple[Sample, dict]], audit: dict, source: str) -> str:
    ...
    lines = [
        "# Evaluation results",
        "",
        f"{len(rows)} samples ({len(genuine)} genuine, {len(attacks)} attacks), local OCR (RapidOCR). "
        f"Source: {source}. Regenerate with `eval/run.py`.",
        "",
        "| Metric | Result |",
        "|---|---|",
